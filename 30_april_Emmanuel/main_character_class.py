...
class main_character:
    def __init__(self):
        ...
        self.sub_rect_bottom_rect_image=self.rect_bottom_image.get_rect(center=(694,460)) #rectangle
        ...
        self.sub_rect_left_rect_image=self.rect_left_image.get_rect(center=(710,442)) #rectangle
    ...
    def collision_main_character(self,bg_rect,bg_mask,dir_up_enable,dir_down_enable,dir_right_enable,dir_left_enable):
        if self.rect_image_mask.overlap(bg_mask,(bg_rect.x-self.rect_rect_image.x,bg_rect.y-self.rect_rect_image.y)) and dir_up_enable==False:
            bg.y-=1
            background.y-=1*.5/3
        if self.rect_image_mask.overlap(bg_mask,(bg_rect.x-self.rect_rect_image.x,bg_rect.y-self.rect_rect_image.y)) and dir_down_enable==False and dir_up_enable:
            bg.y+=.2
            background.y+=.2*.5/3
        if self.rect_image_mask.overlap(bg_mask,(bg_rect.x-self.rect_rect_image.x,bg_rect.y-self.rect_rect_image.y)) and dir_left_enable==False:
            bg.x-=1
            background.x-=1*.5/3
        if self.rect_image_mask.overlap(bg_mask,(bg_rect.x-self.rect_rect_image.x,bg_rect.y-self.rect_rect_image.y)) and dir_right_enable==False and dir_up_enable:
            bg.x+=.2
            background.x+=.2*.5/3
        #bottom_collision
