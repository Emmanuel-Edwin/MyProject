...
if login:
    ...
    class tempchar():
        ...
        y=py1
        @classmethod
        def update_coordinate(cls,px1,py1):
            cls.x=px1
            cls.y=py1
        def __init__(self):
        ...
        def blit(self,screen):
            screen.blit(self.ptemprect_image,(bg.x-tempchar.x+690,bg.y-tempchar.y+440))
    ...
    def instruction_screen():
        ...
        cuo.execute(q)
        d=cuo.fetchall()
        text1=text_font.render(d[0],False,'Red')
        text2=text_font.render(d[1],False,'Red')
        text3=text_font.render(d[2],False,'Red')
        text4=text_font.render(d[3],False,'Red')
        if menu_colour==0:
    ...
    while True:
        ...
        p1_position = client.recv(128).decode()

        
        
        p1_position = ast.literal_eval(p1_position)
        ...
        tempchar.update_coordinate(px1,py1)
        ...
        p2_position = [px2,py2]
        client.send(bytes(str(p2_position), 'utf-8'))

        health1=math.floor(float(client.recv(128).decode()))
        if gs==0:
            client.send(bytes(str(math.floor(health)), 'utf-8'))
        elif gs==1:
            client.send(bytes('you won', 'utf-8'))
        elif gs==2:
            client.send(bytes('opponent won', 'utf-8'))
        #print(health1)
        ...
        if game_status==1:
            ...
            #print(player_rect.colliderect(main_character1.rect_rect_image))
            ...
            if player_rect.colliderect(bullet_rect) and not player_rect.colliderect(gun_surface_rect):#main_character1.rect_rect_image
                if len(Bullet_list)>0:
                    Bullet_list.pop(0)
                if len(Bullet_list)>0:
                    screen.blit(Bullet_list[0],bullet_rect)
        ...
        if gs==0:
            ...
            text_surface3=test_font.render("Health: opponent won",False,'Black')
        elif gs==2:
            text_surface3=test_font.render("Health: you won",False,'Black')
            text_surface3=test_font.render("Health: you won",False,'Black')
        ...
        #screen.blit(player_surface,(bg.x-tempchar.x+690,bg.y-tempchar.y+440))# player_rect
        #char.blit(screen)
        #print(bg.y)

        
        
        m=bg.x-tempchar.x+690
        n=bg.y-tempchar.y+440
        player_rect=player_surface.get_rect(topleft=(bg.x-tempchar.x+690,bg.y-tempchar.y+440))
        #print(m,player_rect.x)
